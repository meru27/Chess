...
public class ArbitroPromocionPeonBlancoADamaBlancaTest {
	...
	*/
	...
	void iniciar() {
		...
		Color[] colores = { NEGRO, BLANCO, NEGRO, NEGRO, BLANCO };
		int[][] posiciones = { { 0, 7 }, { 1, 0 }, { 1, 6 }, { 1, 7 }, { 7, 2 } };
		arbitro.colocarPiezas(tipos, colores, posiciones);
	}
	
	/**
	 * Comprobar promoción de peón blanco a dama blanca.
	 */
	@DisplayName("Comprobar promoción de peón blanco a dama blanca")
	@Test
	void comprobarPromociónDePeonBlancoADamaBlanca() {

		// realizamos movimiento
		Celda origen = tablero.obtenerCelda(1, 0);
		Celda destino = tablero.obtenerCelda(0, 0);

		assertThat("El movimiento se detecta como no legal", arbitro.esMovimientoLegal(origen, destino), is(true));
		assertThat("Numero de piezas negras incorrecto", tablero.obtenerNumeroPiezas(Color.NEGRO), is(3));
	...
	@DisplayName("Comprobar jaque mate al promocionar peón blanco a dama blanca")
	@Test
	void comprobarJaqueMateAlPromocionarPeonBlancoADamaBlanca() {
		...
		arbitro.mover(origen, destino); // mover peón a primera fila promocionando a dama
		...
		// por lo tanto estab en jaque mate y solo queda rendirse
		...
		for (int i = 0; i < ori.length; i++) {
			...
			arbitro.mover(origen, destino); // movimientos de pieza negra
			assertThat("Debería detectarse el jaque", arbitro.estaEnJaque(NEGRO), is(true));
