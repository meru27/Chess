...
public class ArbitroPartidasCompletasTest {
	...
	void comprobarLegalidadDeMovimientosEnJaquePastor() {	
		...
		for (String jugada : JAQUE_PASTOR) {
			...
			if (arbitro.obtenerNumeroJugada()!=7) {
				assertThat("La partida NO debería estar todavía en jaque mate para el rey " + arbitro.obtenerTurno(), !arbitro.estaEnJaqueMate());
			}
		}
		assertThat("El número de jugadas finales es incorrecto", arbitro.obtenerNumeroJugada(),is(7));
		assertThat("La partida debería estar al final en jaque mate para el rey negro", arbitro.estaEnJaqueMate());
	}
